...
- Use `../../scripts/obs_paper.py` with workflow `research-flow` and actions `add_research_flow`, `edit_text`, `move_stream`, `link_literature`, or `remove_items` after inspecting the existing grammar. To change what a card says and nothing else -- a settled term, a polished sentence -- use `edit_text` with the node id; it keeps the kind, colour, and geometry that `add_research_flow` would make you restate, and handles the id stamp itself. Never edit the `.canvas` file by hand to make a card fit: a card past the group's right or bottom edge grows the group.
- A flow that has outgrown one column has two streams. The main column is the argument the paper will make, read top to bottom; the `Appendix` sub-group beside it holds the negative results, ruled-out alternatives, and robustness checks, which stay because they are why the next card exists. Set `"stream": "appendix"` when a card is evidence rather than argument, keep its `y` aligned with the moment in the flow it belongs to, and use `move_stream` when writing the paper changes which side a card is on. Arrows run from the appendix into the main stream, never out of it, and edge colour is assigned from the pair of streams — do not set it by hand.
