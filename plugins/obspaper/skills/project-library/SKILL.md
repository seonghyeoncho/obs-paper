...
Prefer Zotero's local API. Read operations need no account API key. Zotero 10+ write operations request a local key at runtime and the user approves it in Zotero; never store or log that key. Older Zotero releases remain read-only and require either an upgrade or a separately configured zotero.org write API key.

The local API answers on `localhost:23119` only, and Zotero does not support exposing that port, so a machine without Zotero Desktop reaches the library through zotero.org instead. Setting both `ZOTERO_USER_ID` and `ZOTERO_API_KEY` switches the bridge to that web mode; either one alone keeps it local. `status` reports which mode is active and, in web mode, what the key is allowed to do. Web mode covers searching, citation keys, collections, and BibTeX export. It cannot store or read a PDF, because only the desktop holds the files, and it has no Better BibTeX, so citation keys fall back to Zotero's own BibTeX export and can differ from the desktop's. Collect papers and store PDFs on the machine running Zotero. Better BibTeX is optional but preferred for stable LaTeX citation keys. Without it, use Zotero's BibTeX export fallback.
