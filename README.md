...
`nodes` reads cards by exact ID, returning each card's text, group, colour, geometry, and edges on both sides. `edit_text` writes the other way, replacing a card's prose while leaving its kind, colour, and geometry alone, so settling on a term across a flow does not mean re-declaring every card that uses it. A flow also has two streams: the main column reads top to bottom, and an `Appendix` sub-group beside it holds the negative results and robustness checks that cannot be deleted because they are why the next card exists. Edge colour follows the streams an edge joins, arrows out of the main stream into the appendix are refused, and `move_stream` moves a card between the two — keeping its `y`, and repairing the edges around it. Managed research-flow cards print their own ID as their last line, so a card can be addressed without searching the Canvas.
...
| `research-flow` | `add_research_flow`, `edit_text`, `move_stream`, `link_literature`, `remove_items` |
